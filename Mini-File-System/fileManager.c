...
/*make the disk file by make_disk() function and initialize the metadata(DIR, FAT and OFT) 
  ...
  if (make_disk(disk_name) == 0) {
    ...
    printf("Error: failure in making disk\n");
    return -1;
  }
...
//copy data from buf to buffer, then use block_write() to copy data from buffer to the disk file  
    ...
    OFT[fildes][2] = DIR[index][6];
      ...
      if(q != 0 && nbyte > (16 - q) && nbyte <= DIR[index][6] - OFT[fildes][2]){
        for(i = 0; i < p - 1; i++){
          currblock = FAT[currblock - 32][1];
        }
        ...
        for(i = 0; i < m - 1; i++){
          currblock = FAT[currblock - 32][1];
          block_read(currblock, buffer);
          memcpy(buffer, buf + 16 - q + 16 * i, 16);
          block_write(currblock, buffer);
        }
        ...
        block_read(currblock, buffer);
        memcpy(buffer, buf + 16 - q + 16 * (m - 1), nbyte - 16 + q - 16 * (m - 1));
        block_write(currblock, buffer);
      ...
      if(q == 0 && nbyte > 16 && nbyte <= (DIR[index][6] - OFT[fildes][2])){
        for(i = 0; i < p; i++){
          currblock = FAT[currblock - 32][1];
        }
      ...
      if(q == 0 && nbyte > (DIR[index][6] - OFT[fildes][2])){
        currblock = DIR[index][1];
        for(i = 0; i < p - 1; i++){
          currblock = FAT[currblock - 32][1];
        }
      ...
      if(q != 0 && nbyte > DIR[index][6] - OFT[fildes][2]){
        ...
        for(i = 0; i < m - (x - p); i++){
          ...
          if(i == m - (x - p) - 1){
            memcpy(buffer, buf + 16 * (x - p + i) + 16 - q, nbyte - (m - 1) * 16 - 16 + q);
          }else{
            memcpy(buffer, buf + 16 * (x - p + i) + 16 - q, 16);
          }
          block_write(currblock, buffer);
